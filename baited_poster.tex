\documentclass[a0,landscape]{a0poster}
% Кликбейт-версия постера: тот же контент, что в refactored_poster.tex,
% плюс крупный «зазывной» баннер под заголовком. Числа в баннере реальные.

\usepackage{fontspec}
% Кириллицу XeLaTeX берёт прямо из шрифта; polyglossia не используем, т.к.
% масштабирование шрифтов в a0poster (a0size) ломает её проверку OpenType.
\setmainfont{PT Sans}
\tolerance=2000
\emergencystretch=3em
\sloppy

\usepackage{amsmath,amssymb}
\usepackage{multicol}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{array}
\usepackage{xcolor}

\definecolor{titlered}{RGB}{170,30,30}
\definecolor{headblue}{RGB}{0,71,133}
\definecolor{baitbg}{RGB}{255,241,168}
\definecolor{baitedge}{RGB}{210,140,0}

\setlength{\columnsep}{3cm}
\setlength{\parindent}{0pt}
\setlength{\parskip}{0.5\baselineskip}

% синий заголовок секции с линейкой — как в шаблоне
\newcommand{\heading}[1]{\bigskip{\color{headblue}\LARGE\bfseries #1}\par
  \vspace{0.2\baselineskip}{\color{headblue}\hrule height 1.4pt}\medskip}

\newcommand{\R}{\mathbb{R}}
\newcommand{\E}{\mathbb{E}}
\newcommand{\norm}[1]{\lVert#1\rVert}
\DeclareMathOperator{\med}{med}

\begin{document}

% ===================== ШАПКА =====================
{\color{titlered}\veryHuge\bfseries
Beyond Gaussian Noise: Convergence of Stochastic Optimizers\\
under Correlated and Heavy-Tailed Financial Gradient Noise\par}
\medskip
{\Large Д.~Кортунов \quad В.~Пастушенко \quad А.~Дубовицкий \quad А.~Шадрин\par}
\smallskip
{\Large\textit{НИУ ВШЭ, Москва}\par}
\medskip
{\color{titlered}\hrule height 3pt}
\bigskip

% ===================== КЛИКБЕЙТ-БАННЕР =====================
\setlength{\fboxsep}{0.7cm}%
\noindent\colorbox{baitbg}{%
  \parbox{\dimexpr\textwidth-1.4cm\relax}{\centering
    {\LARGE\bfseries\color{titlered}
     Ваш SGD расходится на финансовых данных?}\\[0.3em]
    {\large Одна строка причинной предобработки переводит его
     \textbf{0/8\,$\boldsymbol\to$\,8/8} сходимостей и роняет
     асимптотический $\norm{\nabla f}^2$ \textbf{в 133$\times$}.
     Линейные фильтры так не умеют, а Adam~--- не панацея.
     Какой фильтр выбрать~--- решают \emph{два числа} $(\hat\alpha,\hat H)$.}}}
\par\medskip
\setlength{\fboxsep}{3pt}% вернуть значение по умолчанию

\begin{multicols}{3}

% ----------------------- КОЛОНКА 1 -----------------------
\heading{Постановка}
Наблюдаемый ряд~--- сигнал плюс шум, $y_t = s_t + \xi_t$. Перед обучением
весь ряд проходит через причинный фильтр $F$: $\tilde y = F(y)$, и уже
из сглаженного ряда строятся признаки $\tilde\phi_t$. Обучаем прогнозную
модель (AR($p$)) минимизацией риска методом первого порядка
\[
  g_k=\tfrac1{|\mathcal B_k|}\!\!\sum_{t\in\mathcal B_k}\!\!
      \nabla_x\ell(x_k,\tilde\phi_t,c_t),\qquad
  x_{k+1}=x_k-\eta\,\phi(g_k).
\]
Признаки и обучающий таргет~--- из ряда $\tilde y$; качество же
оценивается на holdout по исходному будущему значению $y_t$.

На финансовых рядах шум тяжёлохвостый ($\alpha$-устойчивый, $\alpha<2$,
бесконечная дисперсия) и долгопамятный. Тогда базовые предпосылки теории
сходимости SGD нарушаются.

\medskip
Вопрос. Как тройка
(тип шума)~$\times$~(фильтр)~$\times$~(оптимизатор) влияет на сходимость,
и можно ли по диагностике ряда заранее выбрать удачную пару?

\heading{Четыре класса шума}
\begin{tabular}{@{}ll@{}}
\toprule
N1 & гауссов белый (контроль)\\
N2 & розовый $1/f$ (долгая память)\\
N3 & $\alpha$-устойчивый (тяжёлый хвост)\\
N4 & смешанный (хвост $\cap$ память)\\
\bottomrule
\end{tabular}

\heading{Фильтры $F_0$–$F_4$ (все причинные)}
\begin{tabular}{@{}ll@{}}
\toprule
$F_0$ & без фильтрации (контроль)\\
$F_1$ & скользящее среднее (линейный)\\
$F_2$ & фильтр Калмана (линейный)\\
$F_3$ & вейвлет-порог (Добеши)\\
$F_4$ & причинная медиана (робастный)\\
\bottomrule
\end{tabular}\par
\smallskip
Оптимизаторы: SGD, Clipped-SGD, Normalized-SGD, Adam, AdamW.
Дизайн~--- два блока: A~(SGD-семейство, N1/N3/N4; спасение сходимости) и
B~(Adam/AdamW, N2/N4; скорость).

% ----------------------- КОЛОНКА 2 -----------------------
\heading{Предпосылки и гипотезы}
При $\alpha<2$ второй момент градиентного шума бесконечен~--- редкие
гигантские шаги не дают SGD стабилизироваться. Медиана устойчива к
выбросам и может вернуть градиентному шуму конечную дисперсию~--- отсюда
спасение. Линейные фильтры $\alpha$-устойчивость не убирают; долгую
память лечит спектральное разреживание (вейвлет) или адаптивный шаг (Adam).
\begin{itemize}\setlength{\itemsep}{0.2\baselineskip}
\item Гипотеза 1 (N3): медиана $F_4$ переводит SGD в сходящийся
      режим; линейные фильтры~--- нет.
\item Гипотеза 2 (N2): для Adam/AdamW вейвлет $F_3$ ускоряет
      сходимость сильнее линейных фильтров.
\item Гипотеза 3 (N4): ни один простой фильтр не снижает
      асимптотический уровень $\|\nabla f\|^2$.
\item Гипотеза 4 (N1): фильтрация нейтральна, медиана чуть хуже
      среднего.
\end{itemize}

\heading{Эксперимент 1: тяжёлые хвосты ($\alpha{=}1.2$, SGD)}
Без фильтра и с линейными $F_1,F_2,F_3$~--- $0/8$ сходимостей; причинная
медиана $F_4$~--- $8/8$, асимптотический уровень $\|\nabla f\|^2$
ниже в $20$–$133\times$.
\begin{center}\small
\begin{tabular}{@{}lcccc@{}}
\toprule
Задача & бин.\,$F_0$ & бин.\,$F_4$ & асимпт.\,$F_0$ & асимпт.\,$F_4$\\
\midrule
квадратичная  & 0/8 & 8/8 & 0.540  & 0.0050\\
логистическая & 0/8 & 8/8 & 0.0658 & 0.00050\\
авторегрессия & 8/8 & 8/8 & 0.0421 & 0.0021\\
\bottomrule
\end{tabular}
\end{center}
\begin{center}
\includegraphics[width=\linewidth]{figures/convergence_rescue.pdf}
\end{center}
\vspace{-0.3\baselineskip}
{\small $F_0$ и линейные фильтры застревают на асимптотическом уровне; $F_4$ уходит
вниз на 1--2~порядка.}

% ----------------------- КОЛОНКА 3 -----------------------
\heading{Долгая память, смешанный шум, гауссов контроль}
N2, Adam: вейвлет $F_3$ ускоряет сходимость в
$11.2\times$ ($T_\varepsilon$ $562{\to}50$), Калман $F_2$~---
$3.1\times$, линейные/медиана~--- нет.
\begin{center}
\includegraphics[width=\linewidth]{figures/longmemory_wavelet.pdf}
\end{center}
\vspace{-0.2\baselineskip}
N4: отношение асимптотических уровней $F_0/F_k\approx1$ для всех
фильтров~--- ни один простой фильтр не работает (гипотеза~3). N1:
фильтрация нейтральна, $F_4$ слегка проигрывает среднему (гипотеза~4):
\begin{center}
\includegraphics[width=\linewidth]{figures/gaussian_control.pdf}
\end{center}

\heading{Эксперименты 2–3: калибровка и реальные ряды}
Медиана по корзине из 9~инструментов: $\hat\alpha\!\approx\!1.2$,
$\hat d\!\approx\!0.29$. На калиброванной синтетике спасение сходимости
сохраняется ($91$–$120\times$):
\begin{center}
\includegraphics[width=\linewidth]{figures/calibrated_synthetic.pdf}
\end{center}
На реальных рядах (Adam); holdout MSE в формате лучший\,/\,$F_0$:
\begin{center}\small
\begin{tabular}{@{}lcccc@{}}
\toprule
Домен & сошл.\,$F_0$ & лучший & ускор. & holdout\\
\midrule
фин.\ 15-мин & 7/16 & $F_2$ & $78\times$ & 0.528/0.522\\
фин.\ дневные & 13/16 & $F_2$ & $10\times$ & 0.634/0.626\\
сенсор ETT & 16/16 & $F_1$ & $2.1\times$ & 0.653/0.488\\
макро FRED & 16/16 & $F_0$ & --- & $F_0$ лучший\\
\bottomrule
\end{tabular}
\end{center}
Фильтр помогает там, где шум тяжёлый, и нейтрален на «аккуратных» рядах.

\heading{Правило $(\hat\alpha,\hat H)\to$ фильтр}
\begin{center}\small
\begin{tabular}{@{}lll@{}}
\toprule
Режим $(\hat\alpha,\hat H)$ & связка & основание\\
\midrule
$\hat\alpha{\approx}2,\ \hat H{\approx}0.5$ & $F_0$ & теория+опыт\\
$\hat\alpha{\approx}2,\ \hat H{>}0.6$       & $F_3{+}$Adam & теория+опыт\\
$\hat\alpha{<}1.9,\ \hat H{\approx}0.5$     & $F_4{+}$SGD & теория+опыт\\
$\hat\alpha{<}1.9,\ \hat H{>}0.6$           & $F_4{/}F_2$, валид. & не проверено\\
\bottomrule
\end{tabular}
\end{center}
$F_2{+}$Adam для N4~--- лишь кандидат, не проверенный напрямую (эвристика).

\heading{Выводы}
\begin{itemize}\setlength{\itemsep}{0.25\baselineskip}
\item Робастная порядковая статистика (медиана) восстанавливает
конечную дисперсию градиента~--- линейные фильтры этого не дают.
\item Тяжёлый хвост сам по себе не повод фильтровать: если узкое
место не в сходимости (гаусс, светлый хвост), лучший выбор~--- $F_0$.
\item На финансовых рядах фильтр ускоряет оптимизацию, но почти не меняет
качество прогноза на holdout.
\item Все четыре гипотезы подтвердились в проведённых экспериментах.
\item Открытая проблема~--- смешанный режим N4.
\end{itemize}

\vfill
{\small Код и данные: \texttt{github.com/Poogee/momo\_proj}}

\end{multicols}
\end{document}
