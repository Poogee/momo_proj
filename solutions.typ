...
$m = 2^12 = 4096$, $n = 400$, $p = 20$. Берём $hat(A) = U "diag"(sigma) V^top$ с ортонормированными $U in R^(m times n)$, $V in R^(n times n)$ и *быстро убывающим спектром* $sigma_i^2 = max(10^(-i\/10), 10^(-12))$: доминантный блок численного ранга $r_("eff")=120$ и вырожденный хвост на полу́. Тогда $kappa(hat(A)^top hat(A)) = 10^12$ — в IEEE double свойство конечной остановки CG (#lec(10), Теорема 1) разрушается и обычный CG *стагнирует*. Быстрое Адамара проверено против рекурсивного определения $H_m$ (порядок 8). Предобусловленный CG (#lec(10), алгоритм):
$ r_0 = b - A x_0, quad z_0 = M r_0, quad d_0 = z_0; quad
  alpha_k = (r_k^top z_k)/(d_k^top A d_k), quad
  beta_k = (r_(k+1)^top z_(k+1))/(r_k^top z_k). $
*Почему $p=20$ достаточно.* Эскиз SRHT имеет $s = n+p = 420 gt.double r_("eff") = 120$ строк: требование к размеру скетча для $(1 plus.minus epsilon)$-вложения масштабируется с *эффективным* рангом, а не с $n$, поэтому $Phi$ почти изометрия на доминантном подпространстве; гребень $10^(-10)"tr"(M^(-1))\/n dot I$ переводит вырожденный хвост во второй плотный кластер. Спектр $M^(1/2) A M^(1/2)$ — фактически два узких кластера $arrow.r.double$ CG сходится сверхлинейно (#lec(10), Теоремы 1–2) за десятки итераций. Полученные числа:
$ kappa(hat(A)^top hat(A)) approx 1.00 dot 10^12, quad
  kappa(M^(1/2) hat(A)^top hat(A) M^(1/2)) approx 34 quad (approx 2.9 dot 10^10 times "меньше"). $
Итерации до относительной невязки: обычный CG достигает $10^(-4)$ за 42, $10^(-6)$ за 195 и *не достигает* $10^(-8)$ за 400 (невязка $approx 3.4 dot 10^(-7)$); PCG — $10^(-8)$ за *43*, $10^(-10)$ за *53* итерации (невязка после 400: $approx 4 dot 10^(-78)$, машинный ноль).

#figure(image("figures/cg_residual.png", width: 95%),
  caption: [Слева: относительная невязка $nm(hat(A)^top hat(b) - hat(A)^top hat(A) x_k)_2 \/ nm(hat(A)^top hat(b))_2$. Справа: число итераций до достижения цели (PCG — десятки, обычный CG не достигает $10^(-8)$ за 400).])
...
*(g) Hessian-Free Newton.* Произведение гессиан-вектор через автодифф $nabla^2 f(w) v = nabla_w [ ip(nabla f(w), v) ]$ (forward-over-reverse, `jax.jvp(jax.grad f)`), гессиан не хранится. Память $O(d)$ против $O(d^2)$. Замеры при $d=100$ (15 итераций, $mu=1$): явный Ньютон $approx 148$ мс, Ньютон–CG (плотный $H$) $approx 405$ мс, HFN $approx 602$ мс. При малом $d=100$ плотный гессиан ($d^2 = 10^4$ чисел) дешевле — overhead автодифф-HVP не окупается.

*(h) Эксперимент масштабирования $d in \{100, 500, 2000, 5000\}$* ($m=2000$, $mu=1$ $arrow.r.double$ $mu$-сильная выпуклость, #lec(11), квадратичная локальная скорость сохраняется; 10 шагов Ньютона). Явный Ньютон формирует и факторизует $d times d$ гессиан ($O(d^2)$ память, $O(d^3)$ решение); HFN не формирует $nabla^2 f$ вовсе. Время (мс):
#align(center)[#table(columns: 5, align: center, inset: 4pt, stroke: 0.4pt,
  table.header([$d$], [память $d^2$], [явный Ньютон], [Ньютон–CG (плотн.)], [HFN]),
  [100],[0.1 МБ],[85],[400],[610],
  [500],[2 МБ],[560],[422],[797],
  [2000],[32 МБ],[3246],[1656],[1843],
  [5000],[200 МБ],[16706],[7995],[*3390*],
)]
Кроссовер при $d approx 500$–700; при $d=5000$ HFN *в $approx 4.9$ раз быстрее* явного Ньютона и хранит $O(d)$ вместо $200$ МБ плотного $d times d$ гессиана. Все методы сходятся до $nm(nabla f) lt.tilde 10^(-8)$. Вывод: $d times d$ гессиан выгоден лишь при малых $d$; на больших $d$ HFN/Ньютон–CG доминируют и по времени, и по памяти.
...
#grid(columns: 2, gutter: 6pt,
  ...
  figure(image("figures/newton_hfn.png"), caption: [(f,g) Ньютон / Ньютон–CG / HFN, $d=100$.]))
#figure(image("figures/newton_scaling.png", width: 95%),
  caption: [(h) Масштабирование по $d$: время (слева, лог-лог) и память гессиана $O(d^2)$ vs $O(d)$ (справа). HFN обгоняет явный Ньютон при больших $d$.])
...
Старт $X_0 = I$ (перестановка). Итерации — выпуклые комбинации матриц перестановок $arrow.r.double$ $X_k in B_n$ *точно* на каждом шаге: численно $max|sum_j X_(i j) - 1| approx 8.9 dot 10^(-16)$, $max|sum_i X_(i j) - 1| approx 6.7 dot 10^(-16)$, отрицательных элементов нет; $f(X_200) approx 6.4132$. Сходимость $O(1\/k)$ (#lec(12), $f(x_k) - f^* <= 2 L R^2 \/ (k+1)$).
...
$kappa(Q) = L\/mu = 100$. Численно: PGD сходится *линейно* (зазор $2.5 dot 10^(-12)$ при старте $e_1$, $2.6 dot 10^(-10)$ из барицентра) — сильная выпуклость. FW *сублинейно* $O(1\/k)$ (зазор $2.4 dot 10^(-3)$ из $e_1$, $5.3 dot 10^(-3)$ из барицентра): оптимум $x^* in "ri"(Delta_n)$ не вершина, FW зигзагует — это проявление нижней оценки $min(n\/2, L R^2 \/ 16 epsilon)$ для методов с оракулом LMO (#lec(12)).
...
$n=2$, $A = mat(1,0;-1,1)$ (невырождена), $Sigma="diag"(0.5,1)$ ($V$ — брус $[-2,2] times [-1,1]$). *Выбор инстанса.* При учебном центре $y=(3,2)$ множества касаются в *единственной точке* $(2,1)$: $U inter V$ имеет пустую внутренность, нет линейной регулярности (оценки Хоффмана), $phi.alt$ *не острая* $arrow.r.double$ лишь $O(1\/k)$ — численно $phi.alt approx 5 dot 10^(-6)$ даже после $2 dot 10^5$ шагов. Ставим эллипсоид в $y=(2.85, 0.9)$, тогда $U inter V$ имеет непустую внутренность (*трансверсальное* пересечение). Линейная регулярность даёт оценку острого минимума $dist(x, U inter V) <= c thin phi.alt(x)$, и шаг Полякa сходится *линейно* к $phi.alt^* = 0$ (#lec(13), шаг Полякa / базовое неравенство). Все старты достигают $phi.alt <= 10^(-8)$ (фактически машинного нуля): $x_0=(0,0)$ — за $k=1$ (глубоко в $V$, рядом с пересечением), $x_0=(2,-1)$ — за $k=25$, $x_0=(1,2)$ — за $k=238$.

=== (5) Обсуждение
- *Выпуклость*: $phi.alt$ выпукла (max выпуклых дистанций), задача выпукла.
- *Гладкость*: $phi.alt$ *негладкая* — изломы там, где $dist(x,U)=dist(x,V)$, и в точках границы (где $dist$ не дифференцируема). В общем случае скорость $O(1\/sqrt(k))$ (#lec(13)); при остром минимуме (трансверсальное $U inter V$) шаг Полякa даёт *линейную* скорость.
- *Единственность*: при учебном $y=(3,2)$ решение единственно (точка касания $(2,1)$); при трансверсальном $y$ — всё множество $U inter V$ оптимально.
- *Скорость по стартам*: старт, лежащий внутри одного множества и близкий к $U inter V$, сходится быстрее всех ($x_0=(0,0)$: $k=1$); $x_0=(2,-1)$ ($k=25$) быстрее, чем дальний $x_0=(1,2)$ ($k=238$) — все три кривые прямые в полулог. масштабе (линейная сходимость).

#figure(image("figures/subgrad_intersection.png", width: 75%),
  caption: [Субградиентный метод (шаг Полякa), трансверсальное $U inter V$: линейная сходимость, $phi.alt^* = 0$; разные старты.])
...
*Реализация* (`notebooks/06_subgrad_lasso.ipynb`): $n=1000$, $m=200$, $lambda=0.01$, разреженный $x^*$ (20 ненулей). Эталон $f^* = 0.1825199063$ получен FISTA ($6 dot 10^4$ итераций, #lec(14)), $nm(x^*)_0 = 119$. За $2 dot 10^4$ итераций зазор $f_k^("best") - f^*$: пост. $1.7 dot 10^(-1)$, пост. длины $1.2 dot 10^(-3)$, $1\/sqrt(k)$ $2.1 dot 10^(-1)$, $1\/k$ $2.6 dot 10^(-1)$ — все $O(1\/sqrt(k))$, а *Поляк* — $bold(2.0 dot 10^(-5))$ (лучшее правило). *Тяжёлый шар* на шаге Полякa $x_(k+1) = x_k - alpha_k g_k + beta(x_k - x_(k-1))$, $beta=0.8$: $bold(4.3 dot 10^(-6))$ — импульс ускоряет ещё $approx 5 times$. Оба «лучших» правила достигают зазора $<= 10^(-4)$.

*Эталон класса* (#lec(14)). Те же $nabla f$, $prox$, но проксимальная структура: проксимальный градиент (ISTA) — $O(1\/k)$, зазор $approx 3 dot 10^(-17)$; FISTA — $O(1\/k^2)$, $approx 6 dot 10^(-17)$ (оба до машинной точности). Разрыв $O(1\/sqrt(k))$ vs $O(1\/k)$ vs $O(1\/k^2)$ нагляден: субградиент застревает на $approx 10^(-5)$, проксимальные методы выходят на машинный ноль (ср. #lec(14), «проксимальный в $sqrt(k)$ раз быстрее субградиентного при равной стоимости итерации»).

#figure(image("figures/subgrad_lasso.png", width: 95%),
  caption: [LASSO. Слева: правила выбора шага субградиента ($O(1\/sqrt(k))$). Справа: лучший субградиент (Поляк, +тяжёлый шар) против проксимального класса — $1\/sqrt(k)$ vs $1\/k$ vs $1\/k^2$ с теоретическими наклонами (пунктир).])
